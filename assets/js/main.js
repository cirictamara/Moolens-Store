...
function scrollFunction() {
	...
	if (document.body.scrollTop > 50 || document.documentElement.scrollTop > 50) {
		document.getElementById("logo").style.cssText = "width: 115px; transform: rotate(15deg); top: 0;";
		document.getElementById("header").style.cssText = "padding: 10px 80px 20px 50px;";
	} 
	else {
		document.getElementById("logo").style.cssText = "width: 160px; transform: rotate(-20deg); top: 35px;";
		document.getElementById("header").style.cssText = "padding: 10px 70px 10px 30px;";
	}
}
...
function createNavigationMenu() {
    ...
    let pages = [ {"name": "Welcome", "href": "#slider"},
    ...
    ul.classList.add("navbar-nav", "mt-3", "ls-1");
...
function logo() {
    let a = document.createElement("a");
    setAttributes([a], ["id", "logo", "class", "navbar-brand position-relative", "href", "#"]);
    a.innerHTML = `<img src = "assets/img/logo.png" alt = "" />`;
    return a;
}

function navigationMenuControls() {
    let button = document.createElement("button");
    setAttributes([button], ["type", "button", "class", "navbar-toggler", "data-toggle", "collapse", "data-target", "navigation-menu", "aria-controls", "navigation-menu", "aria-expanded", "false", "aria-label", "Toggle navigation"]);
...
function createSlides() {
    ...
    for (let i = 0; i < slideContents.length; i++) {
        ...
        setAttributes([img], ["src", `assets/img/${slideImages[i].src}`, "alt", slideImages[i].alt]);
		
        appendChildren(article, [img, createSlideContent(i, slideContents[i])]);
...
function createSlideContent(i, content) {
    ...
    div.classList.add("w-50", "px-4", "position-absolute", "slide-content");
    ...
    switch(i) {
        ...
        case 2:
            ...
            addClasses([h3, p], ["w-70"]);

            if (i == 1) {
                h3.classList.add("fs-35");
                p.classList.add("px-5", "fs-16");
...
function createTextElements() {
    ...
    for (let i = 0, j = 2; i < 2; i++, j += 2) 
        content +=
        `<div class = "w-25 px-6 d-flex flex-column justify-content-center order-${j}">
             <h4 class = "mb-5 fs-25">${divContent[i].h4}</h4>
             <p class = "mb-4">${divContent[i].p}</p>
             <a class = "w-auto arrows" href = "${divContent[i].a.href}">
                 <span class = "position-relative fs-13 ls-1">${divContent[i].a.name} </span>
             </a>
         </div>`;
    return content;
}
    ...
    for (let i = 0; i < 2; i++) {
        switch(i) {
            case 0:
                ...
                content += `<img src = "assets/img/1.jpg" alt = "" />`
                for (let j = 0; j < 2; j++) 
                    content += `<div id = "border-${j + 1}" class = "position-absolute"></div>`;
            break;
            case 1:
                content += "<div>";
                content += `<h2 class = "mb-4 fs-14 ls-2">About Moolen's.</h2>`;
                ...
                `<button id = "our-story" type = "button" class = "ml-6 cursore-pointer arrows">
                     <span class = "fs-14 ls-0">Our Story </span>
                 </button>`;
            break;
        }
        content += "</div>";
...
$(document).ready(function() {
    ...
    `<div id = "modal-body" class = "row-12 position-relative text-center bg-white">
         ...
         <button id = "close" type = "button" class = "position-absolute cursore-pointer">&#10006;</button>
...
gallery.addEventListener("mouseout", () => { 
    ...
    galleryImagesOnHover(0, window.matchMedia("(max-width: 900px)"));
});
...
services.innerHTML =
`<header>
     <h2 class = "fs-40 ls-3">Services</h2>
 </header>
...
function createServices() {
    ...
    for (let serviceList of serviceLists) {
        ...
        `<article id = "service-${serviceList.id}" class = "col-12 col-md-9 col-lg-6 col-xl-5 px-sm-5 mb-0 mb-sm-2 mb-lg-3 service align-self-start text-center">
             <h4 class = "mb-3 mb-md-4 mt-2 mt-lg-0 pb-3 fs-22 ls-2">${serviceList.title}</h4>
...
function createNumberCounters() {
    const numberCounters = [ {"id": 1, "number": 4150, "text": "Quality Products"},
...
function createFooter() {
    ...
    let links = [ {"name": "Sitemap", "href": "sitemap.xml", "margin": "mr-3"},
                  {"name": "Documentation", "href": "documentation.pdf", "margin": "ml-3"} ];
    
    var content = "";

    content += `<section class = "row-12 w-100 mb-sm-5 px-sm-5">`;
    ...
    `<div class = "w-100 mt-3 px-4 px-sm-5 d-flex flex-row flex-wrap justify-content-center justify-content-xl-end align-items-center">
         <p class = "w-100 px-2 pb-3 fs-16 ls-1">Copyright &copy; 2022 The Moolen's Store &verbar; All Rights Reserved</p>`;

    for (let i = 0; i < links.length; i++) 
        content += `<a href = "${links[i].href}" class = "${links[i].margin} mb-2 font-weight-bold font-family fs-10 ls-2">${links[i].name}</a>`;
...
function createSocialMedia() {
    let socialMedia = [ {"href": "https://www.instagram.com", "icon": "fab fa-instagram"}, 
    ...
    for (let i = 0; i < socialMedia.length; i++) 
        ...
        `<li class = "nav-item">
             <a class = "nav-link" href = "${socialMedia[i].href}" target = "_blank">
                 <i class = "${socialMedia[i].icon} fs-16"></i>
             </a>
         </li>`;
    return  content + "</ul>";
}